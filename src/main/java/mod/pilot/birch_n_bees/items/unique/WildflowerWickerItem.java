...
public class WildflowerWickerItem extends BuildableToolBase{
    private static final int craftTime = 1200, checkpoint = craftTime / 2;

    public WildflowerWickerItem(Properties properties) {
    ...
    public void fillValidHeads() {
        ...
        validHeads[0] = new ToolHead(BirchItems.STRIPPED_WILDFLOWER_TWINE.get(), new ItemStack(BirchItems.WILDFLOWER_WICKER.get()));
    }
    ...
    public void trySelectHead(LivingEntity user, ItemStack stack) {
        ...
        if (user.getMainHandItem().equals(stack)) {
            ...
            if (offhand.isEmpty() && user instanceof Player p) {
                ...
                } else messagePlayer(p, "birch_n_bees.message.no_thread_wicker");
            }
            ...
            messagePlayer(p, "birch_n_bees.message.no_offhand_thread_wicker");
        }
    }
